 ...
 */
const createTempFile = async (fileName: string, content: string, encoding: 'utf8' | 'base64' = 'utf8'): Promise<string> => {
  try {
    const tempDir = RNFS.CachesDirectoryPath;
    
    // Sanitize filename to remove invalid characters
    const sanitizedFileName = fileName.replace(/[<>:"/\\|?*]/g, '_');
    const tempFilePath = `${tempDir}/${sanitizedFileName}`;
    
    // Ensure directory exists
    await RNFS.mkdir(tempDir).catch(() => {}); // Ignore error if directory already exists
...
export const exportNoteToDocx = async (note: Note, options: ExportOptions = {}): Promise<string | null> => {
  try {
    ...
    console.log('exportNoteToDocx: Starting export with filename:', rtfFileName);
    ...
    const escapedTitle = (note.title || 'Untitled Note').replace(/[{}\\]/g, '\\$&');
    rtfContent += `{\\b\\fs32\\cf2 ${escapedTitle}}\\par\\par`;
    ...
    console.log('exportNoteToDocx: RTF content created, length:', rtfContent.length);

    // Create temporary file
    const tempFilePath = await createTempFile(rtfFileName, rtfContent, 'utf8');
    console.log('exportNoteToDocx: Temporary file created at:', tempFilePath);
    ...
    console.log('exportNoteToDocx: Save result:', result);

    // Clean up temporary file
    await RNFS.unlink(tempFilePath).catch((error) => {
    ...
    // Handle user cancellation gracefully
    if (error.code === 'DOCUMENT_PICKER_CANCELED') {
      console.log('exportNoteToDocx: User cancelled save dialog');
      return null; // User cancelled, don't show error
    }
    
    // Show more specific error message
    const errorMessage = error.message || 'Unknown error occurred';
    Alert.alert('Export Error', `Failed to export note to document: ${errorMessage}\n\nPlease try again.`);
    return null;
  }
};
